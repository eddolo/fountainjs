...
export async function markdownParagraphRecoveryJourney(page: Page, info: TestInfo): Promise<void> {
  ...
  await verifyFlow(editor);
  await editor.locator('pre').click();
  await page.keyboard.press('End');
  await page.keyboard.type(' changed');
  await expect(editor.locator('pre')).toContainText(' changed');
  await page.keyboard.press('ControlOrMeta+z');
  await verifyFlow(editor);
  await capture('paragraph-flow-editor.png');
  const flowDownload = page.waitForEvent('download');
  await page.getByRole('button', { name: 'Download Markdown draft', exact: true }).click();
  const flowPath = info.outputPath('paragraph-flow.md');
  await (await flowDownload).saveAs(flowPath);
  await page.getByLabel('Open Markdown draft file', { exact: true }).setInputFiles(flowPath);
  await verifyFlow(editor);
  await page.getByRole('button', { name: 'Reader preview', exact: true }).click();
  await verifyFlow(reader);
  await capture('paragraph-flow-reader.png');
  await page.setViewportSize({ width: 390, height: 844 });
  await verifyFlow(reader);
