...
test('uses a host print renderer without moving or exposing live custom DOM', async ({ page }) => {
  ...
  await expect(projection).toHaveAttribute('id', /^fountain-preview-\d+-\d+$/u);
  await expect(projection).not.toHaveAttribute('id', 'host-print-heading');
...
test('runs the public plain-DOM first-class widget demo', async ({ page }) => {
  ...
  const roles = page.getByLabel('How this demo maps to a product');
  await expect(roles).toContainText('Content author');
  await expect(roles).toContainText('Portable document');
  await expect(roles).toContainText('Reader or product user');
  await expect(page.getByText('Developer inspection · not the reader UI')).toBeVisible();

  const block = page.getByRole('group', { name: 'Incident status block' });
  await expect(block).toContainText('Investigating · SEV-2 · Mina');
  await block.getByRole('button', { name: 'Edit attributes' }).click();
  const attributes = block;
  const status = attributes.getByRole('combobox', { name: 'Incident state' });
  const severity = attributes.getByRole('combobox', { name: 'Incident severity' });
  const owner = attributes.getByRole('textbox', { name: 'Incident owner' });
  const label = attributes.getByRole('textbox', { name: 'Status block label' });
  await expect(label).toHaveValue('Incident status');
  await expect(status).toHaveValue('Investigating');
  await expect(severity).toHaveValue('SEV-2');
  await expect(owner).toHaveValue('Mina');
  await status.selectOption('Resolved');
  await severity.selectOption('SEV-1');
  await label.fill('Customer impact');
  await label.press('Tab');
  await owner.fill('Ada');
  await expect(owner).toBeFocused();
  await attributes.getByRole('button', { name: 'Done' }).click();
  await expect(block).toContainText('Resolved · SEV-1 · Ada');
  await expect(attributes.getByRole('textbox', { name: 'Status block label' })).toBeHidden();
  await expect(page.locator('.demo-output pre')).toContainText('"status": "Resolved"');
  await expect(page.locator('.demo-output pre')).toContainText('"label": "Customer impact"');
  await expect(page.locator('.demo-output pre')).toContainText('"severity": "SEV-1"');
  await expect(page.locator('.demo-output pre')).toContainText('"owner": "Ada"');
  await page.locator('.demo-controls').getByRole('button', { name: 'Undo' }).click();
  await expect(block).toContainText('Investigating · SEV-2 · Mina');
  await expect(block).toContainText('Incident status');
  await page.locator('.demo-controls').getByRole('button', { name: 'Incident status' }).click();
  await expect(page.getByRole('group', { name: 'Incident status block' })).toHaveCount(2);
  await expect(page.getByRole('textbox', { name: 'Status block label' })).toBeVisible();
  expect(errors).toEqual([]);
});
...
test('retains table captions and reports their projected association in the conversion demo', async ({ page }) => {
  await page.goto('/demos/node-markdown.html');
  await page.getByLabel('Markdown input', { exact: true }).fill('<table><caption>Trial <strong>results</strong></caption><tr><th>Sample</th><th>Result</th></tr><tr><td>A</td><td>42</td></tr></table>');
  await page.getByRole('checkbox', { name: 'Convert HTML blocks to rich content' }).check();
  ...
  expect(original.content.map((node: { type: string }) => node.type)).toEqual(['paragraph', 'table']);
  await output.getByRole('button', { name: 'html', exact: true }).click();
  await expect(output.locator('pre')).toContainText('<p>Trial <strong>results</strong></p>');
