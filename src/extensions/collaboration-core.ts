...
/**
 * Optional platform renderer for remote selections and carets. The collaboration
 * lifecycle itself is platform-neutral; DOM views install this hook separately.
 */
export type CollaborationPresenceRenderer = (
...
export function createCoreCollaborationExtension(
  options: CollaborationExtensionOptions,
  renderPresence?: CollaborationPresenceRenderer,
): FountainExtension {
  ...
  const plugin = new Plugin<CollaborationState>({
    ...
    props: {
      decorations: renderPresence
        ? (state) => renderPresence(collaborationKey.get(state) ?? initialState(), state.doc)
        : undefined,
      onCreate: (editor) => {
