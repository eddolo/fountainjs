...
`CoreExtension` supplies paragraphs, six heading levels, alignment, quotes, bullet/ordered/task lists, code blocks, tables, media, dividers, semantic hard breaks, links, highlights, text colour, subscript, superscript, and common text marks. Its commands are available both as named imports and through `kit.commands`. Lists support multi-block wrapping, selected-range type conversion, mixed nesting, multi-item indent/lift, ordered starts, task state, boundary joins, and nested HTML/Markdown interchange; the React controls toggle types and expose lift/indent actions. `StarterKit` also adds `LinkBehaviorExtension`: safe URL normalization, host validation hooks, typed web/email autolinking, link-on-paste, whole-link editing around a caret, and an activation event that lets the host decide whether to navigate. The React toolbar exposes add, preview, edit, target, title, and remove controls.
