...
| Framework surfaces | Tiptap provides official bindings above ProseMirror for its supported frontend frameworks | Plain DOM, Web Component, React, and an unreleased first-party Vue entry with lifecycle/SSR/browser tests; Svelte and Angular still use the DOM/Web Component contract | FountainJS is framework-neutral; dedicated Svelte/Angular bindings and equivalent framework-specific UI suites remain open |
