...
| Core packaging | Multiple ProseMirror engine packages plus Tiptap core, extension, framework, UI, and service packages | A compatible browser root plus an import-gated `fountainjs-editor/core` engine entry, isolated optional modules, and `html/server`; core declarations compile without `lib.dom`, and generic/Yjs collaboration execute in pure Node without a fake DOM | Broad engine/package-boundary parity is now demonstrated; ProseMirror/Tiptap still has finer-grained packages and vastly more production use |
...
| Framework surfaces | Tiptap provides official bindings above ProseMirror for its supported frontend frameworks | Plain DOM, Web Component, React, and unreleased first-party Vue/Svelte/Angular entries with lifecycle/SSR/browser tests; Angular uses a compiled standalone directive | FountainJS is framework-neutral; equivalent framework-specific UI suites and broader production integration evidence remain open |
