...
| Framework surfaces | Tiptap provides official bindings above ProseMirror for its supported frontend frameworks | Plain DOM, Web Component, React, and unreleased first-party Vue/Svelte entries with lifecycle/SSR/browser tests; Angular still uses the DOM/Web Component contract | FountainJS is framework-neutral; a dedicated Angular binding and equivalent framework-specific UI suites remain open |
