...
| DOC-11 | Collapsible details/summary content | Partial | The isolated free `details` entry now provides schema-valid details/summary nodes, arbitrary and nested body blocks, public insert/wrap/unwrap/open commands, native accessible disclosure rendering, persisted open state, summary/body keyboard transitions, undo, JSON/HTML/Markdown/text interchange, Yjs synchronization, a live playground route, and unit plus Chromium/mobile browser coverage. Promote after the complete cross-engine release gate and deployed-site verification pass. |
