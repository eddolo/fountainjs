...
| DOC-16 | Invisible-character integrity tools | Missing | Add non-mutating visualization for whitespace, hard breaks, paragraphs, zero-width characters, BOM, NBSP, bidi controls, tabs, and line-ending differences; an accessible raw inspector for code points, UTF-8 bytes, and Unicode normalization; an opt-in verbatim mode that disables text-rewriting features for integrity-sensitive content; and a separate explicit sanitizer with preview/diff and per-category choices. Never silently clean source text. Byte-for-byte source fidelity requires an explicit byte import/hash boundary: a rich DOM editor cannot reconstruct original bytes after the OS, clipboard, decoder, or browser has converted them to text. |
...
| DOC-19 | Office and external-app paste normalization | Partial | Rich HTML now wins over plain-text transforms; supported marks, links, tables, nested lists, extension nodes, images, and spreadsheet grids remain structured; multi-block fragments stay inside compatible table/list containers; and failure falls back safely to text. Add explicit Word, Google Docs, Excel, MathML/LaTeX, ruby, footnote, comment, and tracked-change fixtures plus configurable, reported cleanup policies before marking delivered. Unknown proprietary clipboard properties are not claimed. |
