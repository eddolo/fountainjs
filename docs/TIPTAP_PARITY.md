...
| COLLAB-07 | Lifecycle-safe document/provider replacement and bounded presence traffic | Partial | Generation-scoped contexts, live adapter/`Y.Doc`/provider replacement, stale-session isolation, one-time adapter retirement, default deduplicated/throttled Yjs awareness, twenty-cycle reconnect/listener gates, and twenty-cycle React Strict Mode construction/cleanup gates are implemented locally. Keep Partial until the complete public package and browser CI matrix is green. |
