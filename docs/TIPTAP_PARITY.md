...
| DOC-14 | Page layout, margins, page breaks, headers, and footers | Partial | An isolated DOM-independent foundation covers A4/Letter/custom geometry, deterministic legal-fragment flow, keep-with-next, widow/orphan minima, continuation overhead, footnote reservation, overflow diagnostics, portable manual breaks and footnotes, canonical rich default/first/odd/even header/footer templates, current-page/page-count fields, renderer-neutral per-page furniture/footnote projection, undo, JSON/HTML, and Yjs. A separate browser adapter measures real text lines, direct list items, rowspan-safe table groups, repeated table-header cost, footnotes, templates, and manual breaks without changing the editable DOM; every legal fragment maps back to model/structural paths and clip geometry, and every placement resolves to an exact validated source slice. Its lifecycle controller coalesces mutation/resize/font/print reflow and reuses identity/width/footnote-safe top-level measurements. Repeated middle edits in 1,000 blocks stay below 75 ms p95, while 20 edits alternating between both edges of 5,000 rendered blocks stay below 250 ms p95; every edit performs at most two geometry reads. Six leading insertion/removal cycles preserve all 5,000 unchanged DOM blocks, rebase rendered and cached source paths exactly, stay below 500 ms p95, and require exactly two/one geometry reads. An isolated read-only renderer projects fixed page sheets, line clips, list/table continuation, repeated table headers and furniture, resolved fields, footnotes, physical print rules, and a screen-only accessibility copy without changing the editor or carrying transient selection styling into print. Chromium-generated PDFs verify one page per sheet, A4/Letter MediaBoxes, and page-specific header/field, body, list, table, footnote, and post-break text without hidden duplication. A guarded editable surface places whole top-level sources over fixed sibling page shells while retaining one unchanged contenteditable, direct model paths, DOM identity, native IME, and history. It also keeps one paragraph editable across measured line placements by inserting transient, accessibility-hidden, non-model continuation gaps. Those widgets are excluded from selection mapping, removed before natural remeasurement, and cleaned without accumulating text nodes or observer feedback. Real-browser fixtures prove selection through multiple paragraph gaps, exact page-body alignment, native composition, undo/redo, reversible container-responsive page/continuous modes, tracked insertions and decisions, preserved remote authorship, and bidirectional Yjs convergence across both whole-block and split-paragraph automatic boundaries. It still fails closed with typed issues when a list/table source requires multiple page placements or repeated furniture/footnotes cannot remain uniquely editable. The 363-test package suite and complete 225-check Chromium/Firefox/WebKit/mobile [CI run for `928e45f`](https://github.com/eddolo/fountainjs/actions/runs/33946679970), plus the corresponding [Pages deployment](https://github.com/eddolo/fountainjs/actions/runs/33946679930), certify this guarded editable milestone. Keep this row Partial until split-list/table editing and its review/collaboration cases, repeated furniture/footnote editing, and broader visual/content print fixtures pass immutable CI. See [PAGINATION.md](PAGINATION.md). |
