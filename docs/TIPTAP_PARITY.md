...
| PROD-06 | Extension authoring and compatibility tooling | Partial | Versioned manifests, exact API compatibility, ordered requirements, hard conflicts, a safe package scaffold, a tested framework-neutral example, headless per-extension conformance, and installation-wide diagnostics are implemented locally. Certify the packed CLI, complete package/browser gates, public CI, documentation, and deployed developer guide before marking delivered. |
