...
| DOC-13 | Text style suite: foreground/background colour, font family, font size, and line height | Partial | The complete implementation is present locally: five validated marks, public commands and mixed-selection inspection, safe HTML and lossless browser/headless Markdown interchange, generic Yjs synchronization, an isolated package entry, and responsive React controls. Keep Partial until the complete public CI/browser matrix and deployed playground are verified. |
...
| COLLAB-07 | Lifecycle-safe document/provider replacement and bounded presence traffic | Partial | Connect, disconnect, reconnect, teardown, listener removal, and presence departure are covered. Add repeated React Strict-Mode mount/unmount, live `Y.Doc`/provider replacement, sleep/reconnect simulation, duplicate-listener detection, and presence message-rate/coalescing gates. |
...
| PROD-05 | Performance and memory budgets | Missing | Publish reproducible document-size curves for local and remote transaction latency, input-to-paint, DOM reconciliation, React subscriptions/NodeViews, startup size, retained memory, and teardown. Add complexity gates proving that a small edit or collaboration update does not trigger avoidable whole-document conversion/validation. |
...
| PROD-10 | Practical onboarding and one-layer customization | Partial | The developer guide, source tour, API documentation, ten package-backed demos, and extension examples exist. Add copy-paste starter repositories, task-oriented recipes, extension diagnostics, and independent time-to-first-editor/time-to-first-extension tests so “easier” is measured rather than asserted. |
