...
| DOC-17 | Stable unique node identifiers | Partial | The isolated DOM-free module now provides configurable `nodeId` policy, deterministic/injected generation, O(1) lookup, fail-closed diagnostics, lookup/update/select commands, one-step history-neutral repair, JSON normalization, schema filtering, and mixed-client Yjs repair convergence. Complete package and browser certification is pending before this row becomes Delivered. |
