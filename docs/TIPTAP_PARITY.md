...
| DOC-14 | Page layout, margins, page breaks, headers, and footers | Partial | An isolated DOM-independent foundation covers A4/Letter/custom geometry, deterministic legal-fragment flow, keep-with-next, widow/orphan minima, continuation overhead, footnote reservation, overflow diagnostics, portable manual breaks and footnotes, canonical rich default/first/odd/even header/footer templates, current-page/page-count fields, renderer-neutral per-page furniture/footnote projection, undo, JSON/HTML, and Yjs. A separate browser adapter measures real text lines, direct list items, transitive rowspan-safe table groups, safe multi-row repeated headers, footnotes, templates, and manual breaks without changing the editable DOM; every legal fragment maps back to model/structural paths and clip geometry, and every placement resolves to an exact validated source slice. Its lifecycle controller coalesces mutation/resize/font/print reflow and reuses identity/width/footnote-safe top-level measurements. Repeated middle edits in 1,000 blocks stay below 75 ms p95, while 20 edits alternating between both edges of 5,000 rendered blocks stay below 250 ms p95; every edit performs at most two geometry reads. Six leading insertion/removal cycles preserve all 5,000 unchanged DOM blocks, rebase rendered and cached source paths exactly, stay below 500 ms p95, and require exactly two/one geometry reads. An isolated read-only renderer projects fixed page sheets, line clips, list/table continuation, repeated table headers and furniture, resolved fields, footnotes, physical print rules, and a screen-only accessibility copy without changing the editor or carrying transient selection styling into print. Chromium-generated PDFs verify one page per sheet, A4/Letter MediaBoxes, and page-specific header/field, body, list, table, footnote, and post-break text without hidden duplication. A guarded editable surface preserves one contenteditable and the canonical DOM/model nodes while continuing measured paragraph lines, list items, and transitive rowspan-safe table row groups through transient non-model spacing. Split tables remain one editable table, with read-only accessibility-hidden column-header copies in later page shells; multi-row headers preserve safe row/column spans, while a header rowspan entering body rows suppresses the incomplete repeated copy. Canonical page templates and footnote definitions remain uniquely editable in ordered rails around the sheet stack; sanitized, accessibility-hidden copies resolve fields and page-local notes without model paths, IDs, or editable controls. Unsplittable rows, images, media, details, code, and custom NodeViews remain canonical and editable: they move intact when they fit and use explicit non-clipping overflow when taller than the body. Pagination-owned attributes and CSS-variable changes preserve custom NodeView identity without weakening recovery for unrelated DOM mutations. Real-browser fixtures prove cross-page selection, composition, undo/redo, exact alignment, stable reflow, responsive fallback/restoration, tracked decisions, mapped comments, top-level movement, preserved remote authorship, bidirectional Yjs convergence across whole-block, paragraph, list, and table boundaries, and representative atomic/custom-NodeView interaction and history. Unsupported structural fragments and invalid page-intent order fail closed. The latest immutable public certification is the 367-test package suite plus the whole-block, paragraph, list, table, mapped-comment, movement, oversized-row, page-intent, and atomic/custom-NodeView browser matrix in [CI run `4e60bed`](https://github.com/eddolo/fountainjs/actions/runs/33953036708), plus the corresponding [Pages deployment](https://github.com/eddolo/fountainjs/actions/runs/33953036702). Complex merged-table evidence is the next immutable gate. Keep this row Partial until broader adversarial/cross-engine visual and content print fixtures pass immutable CI. See [PAGINATION.md](PAGINATION.md). |
