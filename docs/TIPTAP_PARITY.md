...
| UI-04 | Drag handles and block reordering | Partial | Path-based schema-valid nested/cross-parent moves, visible contextual handles, real drop indicators, touch/keyboard buttons, host policy, one-step undo, package-backed demos, and local desktop/mobile contracts are implemented; promote only after the exact all-engine CI and deployed-site release evidence pass. |
