...
| CORE-12 | Granular structured node attributes | Partial | DOM-free bounded definitions and typed path commands, stable-ID-addressed nested Yjs maps/arrays, non-overlapping field and concurrent-insertion convergence, canonical JSON repair, local-only undo, fail-closed remote validation, package entries, documentation, and a public two-editor control surface are implemented and focused tests pass. This row remains Partial until the complete package/browser matrix and deployed build are recorded. |
