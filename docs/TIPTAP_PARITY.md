...
| PROD-07 | Stable releases and migrations | Partial | The source now contains explicit API stability/deprecation/security-support/rollback rules, a DOM-free document envelope and sequential migration runner, a published structural JSON Schema, host schema validation, package smoke/budget/type coverage, and machine-checked version/tag/changelog metadata in the trusted-publisher workflow. Complete the full local and public package/browser gates, verify the deployed guidance, and attach immutable run links before marking this delivered. |
