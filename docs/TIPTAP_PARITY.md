...
| COLLAB-04 | Threaded inline and document comments | Partial | Provider-neutral thread records and lifecycle; inline/cross-block, point, block, and document anchors; overlapping decorations; mapped movement, recovery/orphan reattachment; replies, rich bodies, editing, reactions, resolve/archive/delete; permission hooks; an authoritative storage adapter plus in-memory reference; accessible React UI; package boundaries; documentation; and focused unit/Chromium/WebKit tests are implemented locally. Promote only after the complete CI/browser gate, Pages deployment, and live deployed interaction are verified. |
