...
| CORE-11 | First-class portable interactive widgets | Partial | The DOM-free definition/controller, validated one-step state updates, protected identity, explicit focus/key exits, read-only behavior, safe bounded interchange, generic history/Yjs behavior, separate DOM and React renderers, and two public demo routes are implemented. Mark Delivered only after the complete package and Chromium/Firefox/WebKit/mobile release gates plus Pages deployment certify the public package. |
