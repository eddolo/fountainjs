...
| DOC-14 | Page layout, margins, page breaks, headers, and footers | Partial | An isolated DOM-independent foundation covers A4/Letter/custom geometry, deterministic legal-fragment flow, keep-with-next, widow/orphan minima, continuation overhead, footnote reservation, overflow diagnostics, portable manual breaks and footnotes, canonical rich default/first/odd/even header/footer templates, current-page/page-count fields, undo, JSON/HTML, and Yjs. A separate browser adapter measures real text lines, direct list items, rowspan-safe table groups, repeated table-header cost, footnotes, templates, and manual breaks without changing the editable DOM; its lifecycle controller coalesces and times mutation/resize/font/print reflow with explicit teardown. Its initial 339-test package suite and complete 193-check Chromium/Firefox/WebKit/mobile [CI run for `fc33455`](https://github.com/eddolo/fountainjs/actions/runs/33932531158), plus the corresponding [Pages deployment](https://github.com/eddolo/fountainjs/actions/runs/33932531264), are green. Add editable page shells, repeated template projection, continuation rendering, cross-boundary selection/IME/review, continuous accessibility, larger incremental reflow budgets, and print/PDF fixtures before calling pagination delivered. See [PAGINATION.md](PAGINATION.md). |
