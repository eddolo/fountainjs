...
| DOC-14 | Page layout, margins, page breaks, headers, and footers | Partial | The isolated neutral foundation, DOM measurement/controller, read-only preview, and guarded editable surface now cover A4/Letter/custom geometry; legal fragment flow; portable breaks, footnotes, templates, and fields; real line/list/table measurement; exact source projection; bounded incremental reflow; canonical whole-block/paragraph/list/rowspan-safe table continuation; safe merged multi-row header copies; unique editable page-intent rails; responsive fallback; explicit oversized row/atomic overflow; and retained selection, IME, history, review, comments, movement, custom NodeViews, and Yjs. Chromium, Firefox, and WebKit verify physical A4/Letter sheet geometry, stable named-page assignment, headers/page numbers, page-local footnotes, forced print breaks, accessibility isolation, and removal of transient editor state; Chromium additionally verifies emitted PDF page count, MediaBoxes, and page-specific extracted content. The latest immutable public certification is the 369-test package suite and complete browser matrix through physical print projection in [CI run `678886a`](https://github.com/eddolo/fountainjs/actions/runs/33954387906), plus the corresponding [Pages deployment](https://github.com/eddolo/fountainjs/actions/runs/33954387903). Gap-adjacent caret/range/IME and mobile-fallback evidence is the next immutable gate. Keep this row Partial until broader adversarial visual/content print fixtures pass immutable CI. See [PAGINATION.md](PAGINATION.md). |
