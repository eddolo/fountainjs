...
| PROD-05 | Performance and memory budgets | Partial | Reproducible production-build curves now cover local, incremental-remote, and untrusted full-JSON updates through 10,000 blocks; heap gates cover a 2,000-edit live session and forty destroyed editors; raw startup-size ceilings remain enforced; immutable subtree validation, top-level DOM reconciliation, zero-churn React NodeViews, direct remote transactions, and text-only Yjs deltas prevent avoidable whole-document work. Chromium/WebKit pass the new 1,000-block input-to-paint and 999/1,000 DOM-identity gate locally. Mark Delivered after the complete public Chromium/Firefox/WebKit/mobile CI matrix certifies the change; methodology and limitations are in [the performance contract](PERFORMANCE.md). |
