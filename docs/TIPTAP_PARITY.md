...
| SURFACE-05 | Headless/server document processing | Partial | DOM-free model, transactions, collaboration, JSON/text/Markdown/HTML export, and the certified `html/server` importer run today. Publish a clean no-`DOM` core declaration entry and separate the remaining mixed extension/view contracts before calling the entire headless surface delivered. |
