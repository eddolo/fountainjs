    ...
    "content": [
      ...
      { "type": "text", "text": "and typed", "marks": [{ "type": "strong" }] }
          ...
          <section className="dev-section" id="extensions">
            ...
            <p>Select a formula to edit its source directly. Display math accepts Enter for new lines and Ctrl+Enter or Command+Enter to finish; inline math uses Enter to finish and Shift+Enter for a new line. Undo restores live edits. Multiline source and unchanged accessibility descriptions survive inspection. The <a href="./math-renderer.html">real KaTeX renderer lab</a> demonstrates typesetting, editable source fallback, and the published equations this renderer still rejects. These controls edit formulas; whole-paper TeX layout and bibliography reproduction remain under the <a href="https://github.com/eddolo/fountainjs/blob/master/docs/REFERENCE_DOCUMENT_AUDIT.md">real-document audit</a>.</p>
            <p>The <a href="./math-references.html">document-aware equation lab</a> uses a caller-owned MathJax SVG adapter through <code>documentRenderer</code>. It keeps the original labels, numbers and resolves references across blocks, refreshes on reorder/deletion/undo, and reports compilation and font fallback diagnostics. Its reader preview has independent link anchors. Local JSON/Markdown downloads and validated reopening demonstrate <code>replaceDocument</code>, including root metadata and undo; external assets and session history are not bundled. Use <strong>Build page preview</strong> to try <code>layoutDOMPages</code> and <code>renderDOMPagePreview</code>: cross-page SVG links stay inside the snapshot, which is marked stale after editing. The bounded example uses locally bundled TeX font data; <a href="./mathjax-notices.txt">renderer and font notices</a> describe its third-party licenses. This is not full-paper or PDF/DOCX export parity.</p>
            ...
            <p>The equation lab’s <strong>Print / Save PDF</strong> uses a current paged snapshot and host print CSS. A recorded Chromium export has independently inspected pages and verified internal PDF equation destinations; Firefox/WebKit have print-layout coverage, not equivalent native PDF evidence. Tagged accessible PDF, original-paper layout and native DOCX equations remain open. DOCX currently preserves TeX as explicitly lossy fallback text.</p>
          ...
          <section className="dev-section" id="surfaces">
            ...
            <div className="dev-two-column dev-two-column--cards">
              ...
              <div><h3>Web Component</h3><p>A standards boundary with a <code>value</code> property, <code>fountain-change</code> event, and configurable schema/plugins. Opt in with <code>formAssociated: true</code> for native JSON form submission, reset and fieldset disabling. <a href="./demos/java-approval-workflow.html#native-form">Try the form workshop →</a> <a href="https://github.com/eddolo/fountainjs/blob/master/docs/WEB_COMPONENT_FORMS.md">Read the integration contract →</a></p></div>
              <div><h3>React</h3><p>Hooks, composer, toolbar, navigator, clipboard picker, accessible suggestion/slash/bubble/floating menus and character count, plus optional AI review, conversation, and generated-media UI from <code>fountainjs-editor/react</code>. Discussions, tracked review, versions, and text-integrity inspection are isolated in <code>fountainjs-editor/react/comments</code>, <code>fountainjs-editor/react/tracked-changes</code>, <code>fountainjs-editor/react/versions</code>, and <code>fountainjs-editor/react/integrity</code>.</p></div>
              <div><h3>Vue 3</h3><p><code>fountainjs-editor/vue</code> provides <code>useFountain</code>, <code>useFountainState</code>, and <code>FountainEditor</code>. The engine is created on client mount, tracked without deep proxies, and disposed by its owner; the view can hide/reopen without losing history. Try the <a href="./demos/vue-runbook.html">real Vue runbook</a> and read the <a href="https://github.com/eddolo/fountainjs/blob/master/docs/VUE.md">lifecycle, SSR, options and ownership guide</a>.</p></div>
              <div><h3>Svelte 5</h3><p><code>fountainjs-editor/svelte</code> provides <code>createFountain</code>, <code>fountainState</code>, and the <code>fountainEditor</code> action. A client-owned editor feeds readable stores; hiding its DOM host retains history. Try the <a href="./demos/svelte-report.html">compiled Svelte report</a> and read the <a href="https://github.com/eddolo/fountainjs/blob/master/docs/SVELTE.md">ownership, SSR and action guide</a>. Available in the repository build; npm publication is still gated.</p></div>
              <div><h3>Angular 22</h3><p><code>fountainjs-editor/angular</code> provides <code>createFountain</code>, <code>fountainState</code> and the standalone <code>FountainEditorDirective</code>. Client-owned editors feed signals; the directive owns only the view. Try the <a href="./demos/angular-media.html">compiled Angular campaign</a> and read the <a href="https://github.com/eddolo/fountainjs/blob/master/docs/ANGULAR.md">ownership, partial compilation, SSR and upload guide</a>. Available in the repository build; npm publication is still gated.</p></div>
              <div><h3>Your framework</h3><p>The DOM API and Web Component remain available. Create one editor, subscribe on mount, dispatch framework-neutral commands, and dispose owned views/editors when their session ends.</p></div>
            </div>
          ...
          <section className="dev-section" id="formats-media">
            ...
            <p>To retain ordinary section wrappers, explicitly add <code>HTMLContainerExtension</code> with <code>composeExtensions</code>. Its <code>html_container</code> node preserves supported div/section-style containers and bounded ID, class, title and language/direction attributes. Low-priority rules yield to your application-specific parsers; unknown attributes decline the wrapper with server conversion details. The <strong>Preserve HTML section containers</strong> option in the conversion demo installs this extension without changing StarterKit or enabling Markdown HTML automatically. See the <a href="https://github.com/eddolo/fountainjs/blob/master/docs/HTML_CONTAINERS.md">API, recorded workflow and remaining limits</a>.</p>
            <p>The same optional module supplies <code>insertHTMLContainer</code>, <code>appendHTMLContainerParagraph</code> and <code>unwrapHTMLContainer</code>. Use the existing <code>setNodeAttributes</code> command for properties. Imported empty wrappers stay empty until the author explicitly adds a paragraph; removing a wrapper keeps its children. The <a href="./demos/node-markdown.html#section-authoring">section authoring workshop</a> connects these APIs to labelled controls, undo/redo and a separate reader snapshot. Its outlines are demo CSS, not stored content or a mandatory module theme.</p>
            <p>For raw HTML scopes that cross paragraph or list boundaries, <code>parseHTMLDocument: ServerHTMLImporter.parseTextBlockFlow</code> performs one root source projection before any local HTML adapter. It takes precedence over the block/flow/paragraph/inline callbacks and retains the complete inert document on refusal. Exact unchanged source survives. After document HTML conversion, edited documents use canonical Markdown rather than unsafe independent block splicing. Ordinary Markdown still retains independently captured source blocks when no document conversion runs. The separate combined-container profile matches 599/652 reference cases; repaired whitespace paragraphs can still change spacing. Read the <a href="https://github.com/eddolo/fountainjs/blob/master/docs/MARKDOWN_DOCUMENT_FLOW.md">whole-document contract, recorded comparison and open layout gap</a>.</p>
            ...
            <p>Markdown can opt into the same importer through <code>MarkdownImportOptions.parseHTMLBlock</code>. The core still bundles no HTML parser: your synchronous adapter converts raw HTML blocks into the supplied schema, while <code>onHTMLBlockFallback</code> reports conversions retained as literal text. Nested containers and source snapshots share that policy. For rich table export, <code>tableFormat: 'html'</code> preserves supported cell blocks and layout through that same boundary; pipe tables remain the default. The demo’s Markdown panel exposes both the choice and its compatibility/loss notes. Exact-source export is not sanitization. Read the <a href="https://github.com/eddolo/fountainjs/blob/master/docs/MARKDOWN_SOURCE.md">adapter and source-preservation contract</a>.</p>
            <p>The separate <code>parseHTMLInline</code> option receives raw HTML tokens alongside original Fountain nodes. Connect it to <code>ServerHTMLImporter.parseInline</code>, or the instance <code>parseInlineWithReport</code> method for loss details. HTML scopes can add formatting without rebuilding Markdown nodes through HTML; protected-node checks reject content loss or reordering. Failed conversions remain readable source and notify <code>onHTMLInlineFallback</code>. Reports distinguish omitted comments, unmapped inline elements, and rejected link/image URLs without echoing private source payloads. Try <strong>Convert inline HTML formatting</strong> in the conversion demo. Both HTML options are off by default. Specialized inline structures and full CommonMark HTML conformance remain unfinished.</p>
            <p>For block tags embedded inside an ordinary paragraph, the experimental <code>parseHTMLParagraph</code> callback can return several blocks. Connect it to <code>ServerHTMLImporter.parseParagraph</code> or <code>parseParagraphWithReport</code>; forward the third <code>context</code> argument so tight-list paragraphs omit their implicit HTML wrapper. Forward segments unchanged, including <code>softBreak</code> provenance: closed text-only preformatted scopes can then restore line breaks. Their conversion report explains HTML newline handling and plain-text code exports. Use <code>onHTMLParagraphFallback</code> for rejected conversions. It takes precedence over inline conversion only in ordinary paragraphs, not headings or pipe-table cells. Try <strong>Recover block tags inside paragraphs</strong> in the conversion demo. This option is off by default; atoms, cross-paragraph preformatted scopes and active raw-text remain unsupported, and unknown HTML identity is not lossless. See the <a href="https://github.com/eddolo/fountainjs/blob/master/docs/MARKDOWN_SOURCE.md#experimental-paragraph-html-recovery">exact contract and remaining limits</a>.</p>
            ...
            <p>Math export is a separate experimental boundary: <code>resolveMath(node, path)</code> may supply a validated semantic expression for native Word OMML, with original TeX retained in package metadata. It does not select a TeX parser. Without a supported projection, export keeps readable TeX and reports the loss. Word/LibreOffice rendering and editing, source restoration, and live equation numbering/references are not yet verified or implemented. Imports explicitly identify unsupported Office equations instead of flattening them into misleading text.</p>
          ...
          <section className="dev-section" id="ai-mcp">
            ...
            <h3>Conversation history belongs to the host</h3>
            <p><code>fountainjs-editor/ai/conversation</code> adds bounded, inspectable multi-turn context and reusable prompts without choosing a model, database, or account system. Streaming replies remain transient until the host saves them through its conversation adapter; cancellation, idempotent saves, optimistic conflict checks, guarded clearing, and immutable snapshots are part of the contract. The homepage’s <a href="./#playground">optional AI lab</a> exercises a prompt, first reply, follow-up context, and clear flow. See the <a href="https://github.com/eddolo/fountainjs/blob/master/docs/AI_CONVERSATIONS.md">conversation guide</a>.</p>
            <h3>Generated media returns through ordinary uploads</h3>
            <p><code>fountainjs-editor/ai/generated-media</code> creates a private-by-default, inspectable request and bounded byte-backed image, audio, video, or file candidates. Candidates stay outside the document until a person accepts one. The browser bridge then creates a normal <code>File</code> and uses the same host-owned image or asset upload path as paste, drop, and file selection, preserving progress, cancellation, mapped insertion, schema validation, and one-step undo. The homepage lab shows the complete request → preview → upload → insert → undo journey with a deterministic local generator and no network model call. See the <a href="https://github.com/eddolo/fountainjs/blob/master/docs/AI_GENERATED_MEDIA.md">generated-media guide</a>.</p>
            <div className="dev-callout"><b>Privacy posture</b><span>Text review sends the selected text and minimal action context—not the whole document. Conversation sends only the bounded context the host assembles. Generated-media requests exclude document content and reference assets by default. In every case the host owns the adapter, endpoint, authentication, logging, and retention policy.</span></div>
          </section>
          ...
          <section className="dev-section" id="source-tour">
            ...
            <div className="source-list">
              ...
              <a href="https://github.com/eddolo/fountainjs/tree/master/src/ai"><code>src/ai/</code><span>Provider-neutral review, streaming, schema-aware document tools, host-owned conversations, generated-media review, and MCP adapter.</span></a>
              <a href="https://github.com/eddolo/fountainjs/blob/master/docs/AI_CONVERSATIONS.md"><code>docs/AI_CONVERSATIONS.md</code><span>Conversation storage, context assembly, prompt templates, streaming lifecycle, React UI, and trust boundaries.</span></a>
              <a href="https://github.com/eddolo/fountainjs/blob/master/docs/AI_GENERATED_MEDIA.md"><code>docs/AI_GENERATED_MEDIA.md</code><span>Generation requests, byte-backed review, upload acceptance, provenance, limits, React UI, and security boundaries.</span></a>
