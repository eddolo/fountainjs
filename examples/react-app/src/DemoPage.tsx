  ...
  return <div className="demo-workspace">
    <section className="demo-surface headless-surface"><div className="surface-label"><span>LIVE HEADLESS FORMAT PIPELINE</span><i>This conversion panel uses no contenteditable or EditorView.</i></div><nav className="headless-input-tabs" aria-label="Headless input format"><button className={inputFormat === 'markdown' ? 'active' : ''} onClick={() => setInputFormat('markdown')}>Markdown</button><button className={inputFormat === 'html' ? 'active' : ''} onClick={() => setInputFormat('html')}>Server HTML</button><button className={inputFormat === 'docx' ? 'active' : ''} onClick={() => setInputFormat('docx')}>Word DOCX</button></nav>
      ...
      {inputFormat === 'markdown' && <div className="headless-html-policy">
        ...
        {projectDocumentFlow && <p>Experimental whole-document source conversion replaces the four local HTML options below. Inline HTML can carry across paragraphs and supported lists/quotes. Unsupported structures or active raw-text HTML cause an all-inert fallback; no partial conversion is kept. Source capture retains an untouched file exactly. Edits after document HTML conversion use canonical Markdown; ordinary Markdown still retains independent source blocks when no conversion runs. HTML repair can create extra formatted whitespace paragraphs: read the layout warning. <a href="https://github.com/eddolo/fountainjs/blob/master/docs/MARKDOWN_DOCUMENT_FLOW.md">Contract, examples and remaining gaps →</a></p>}
